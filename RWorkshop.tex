...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#create a wide data format of surveys using spread}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#now we want to spread the dataset out into a wide format}

\NormalTok{wide_surveys_gw <-}\StringTok{ }\NormalTok{surveys_gw }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{spread}\NormalTok{(}\DataTypeTok{key=}\NormalTok{species_id, }\DataTypeTok{value=}\NormalTok{mean_weight)}
\CommentTok{#long format is usually better, group together species for analysis}

\CommentTok{#now we are going back to long from the wide dataset that we just created}
\end{Highlighting}
\end{Shaded}
...
\item
  value
\item
  names of columns we use to fill the key variable (or drop)
\end{enumerate}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{long_surveys <-}\StringTok{ }\NormalTok{wide_surveys_gw }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{gather}\NormalTok{(}\DataTypeTok{key=}\NormalTok{species_id, }\DataTypeTok{value=}\NormalTok{mean_weight)}
\end{Highlighting}
\end{Shaded}
